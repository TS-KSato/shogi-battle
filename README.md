...
スマートフォンの縦持ちに合わせて作っている。パソコンなど横長の画面では、
中央にスマートフォンと同じ縦横比（390:844）の枠を置いてその中に表示する。
...
対局画面が初回に読むファイル（画面・スクリプト・舞台絵・GLB 23個。退避用の2個と
CDN の three.js は除く）の合計は 5.77 MB で、gzip -9 後は 3.81 MB。そのうち兵の GLB 20体が 5.45 MB
（gzip 後 3.57 MB）で、9割を占める。

gzip 後の合計を 4 Mbps で割ると約 8 秒。これは転送量だけの概算で、実測ではない
（GLB が gzip なしで配られる場合は実体の 5.77 MB で約 12 秒）。タイトルを経由すると
待機の GLB 16体（進化後の4体を除く）が先読みされるので、対局画面ではその分を待たずに
済む。タイトル側の絵とロゴは 0.5 MB。
